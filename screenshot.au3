...
Global $seconds = 5 				;seconds between each screenshot
...
Global $programToActivate = "Photoshop"	;title of the window that must be active to take a screenshot. Leave blank to always be true
;##### USER CONFIG #####
...
Init()
If $quality > 100 Or $quality < 1 Then Exit @ScriptLineNumber
...
Func SS()
	If WinActive($programToActivate) <> 0 And FileExists(@DesktopDir & "\" & $FolderName) Then
		...
		$hBmp = _ScreenCapture_Capture("")
		_ScreenCapture_SaveImage(@DesktopDir & "\" & $FolderName & "\SS" & $num & ".jpg", $hBmp)
	ElseIf FileExists(@DesktopDir & "\" & $FolderName) == 0 Then
